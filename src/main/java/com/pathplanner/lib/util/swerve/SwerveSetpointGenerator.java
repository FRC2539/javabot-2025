...
public class SwerveSetpointGenerator {
    ...
    public SwerveSetpoint generateSetpoint(
        ...
        for (int m = 0; m < config.numModules; m++) {
            ...
            double currentDraw =
                    config.moduleConfig.driveMotor.getCurrent(
                            Math.abs(lastVelRadPerSec), inputVoltage);
            ...
            reverseCurrentDraw =
                    Math.min(reverseCurrentDraw, config.moduleConfig.driveCurrentLimit);
            double forwardModuleTorque = config.moduleConfig.driveMotor.getTorque(currentDraw);
            double reverseModuleTorque =
                    config.moduleConfig.driveMotor.getTorque(reverseCurrentDraw);
        ...
        ChassisSpeeds chassisAccel =
                new ChassisSpeeds(
                        chassisAccelVec.getX(), chassisAccelVec.getY(), chassisAngularAccel);
