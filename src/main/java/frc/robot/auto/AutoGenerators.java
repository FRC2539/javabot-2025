...
import frc.robot.auto.Auto.DriveLocation;
...
public class AutoGenerators {
    ...
    public static class AutoGeneratorRobot extends RobotBase {
        ...
        public AutoGeneratorRobot() {
            ...
            try {
                ...
                final PathTweakableBezier.Tweaker standardTweak =
                        (input, rand) -> {
                            List<Pose2d> newPoints = new ArrayList<>(input.points);
                            int point = rand.nextInt(0, newPoints.size());
                            for (int i = 0; i < newPoints.size(); i++) {
                                if (i != point) continue;
                                newPoints.set(
                                        i,
                                        PathTweakableBezier.tweakPoseRotation(
                                                newPoints.get(i), rand, 0.2));
                            }
                            for (int i = 1; i < newPoints.size() - 1; i++) {
                                if (i != point) continue;
                                newPoints.set(
                                        i,
                                        PathTweakableBezier.tweakPoseTranslation(
                                                newPoints.get(i), rand, 0.2));
                            }
                            return new PathTweakableBezier(
                                    input.startState, input.endState, newPoints, input.tweaker);
                        };
                
                final PathTweakableBezier.Tweaker preserveFinalDirection =
                        (input, rand) -> {
                            List<Pose2d> newPoints = new ArrayList<>(input.points);
                            int point = rand.nextInt(0, newPoints.size());
                ...
                for (var value : pairs) {
                    ...
                    var end =
                            getPoseFromTag(value[1])
                                    .plus(new Transform2d(-0.3, 0, Rotation2d.kZero));
                    var yay =
                            new PathTweakableBezier(
                                    new IdealStartingState(0, start.getRotation()),
                                    new GoalEndState(2, end.getRotation()),
                                    subdividePoses(start, end, 0),
                                    preserveFinalDirection);
                ...
                //         var end =
                //                 getPoseFromTag(value)
                //                         .plus(new Transform2d(-0.3, 0, Rotation2d.kZero));
                //         PathTweakable yay =
                //                 new PathTweakable(
                //                         start,
                //                         start.interpolate(end, 0.3).getTranslation(),
                //                         end.plus(new Transform2d(-1, 0, Rotation2d.kZero))
                //                                 .getTranslation(),
                //                         end);
    ...
    private static List<Pose2d> subdividePoses(Pose2d start, Pose2d end, int subdivisions) {
        subdivisions++;
        List<Pose2d> output = new ArrayList<>();
        for (double i = 0.0; i <= subdivisions; i++) {
            output.add(start.interpolate(end, i / subdivisions));
        }
        return output;
    }
}
