 ...
 *
 * <p>Check <a
 * href='https://shenzhen-robotics-alliance.github.io/maple-sim/swerve-simulation-overview/'>Online
 * Documentation</a>
 *
 * <h3>1. Purpose</h3>
 *
...
public class SwerveDriveSimulation extends AbstractDriveTrainSimulation {
     ...
     * field.
     *
     * @param config a {@link DriveTrainSimulationConfig} instance containing the configurations of
     *     * this drivetrain
    ...
    public SwerveDriveSimulation(DriveTrainSimulationConfig config, Pose2d initialPoseOnField) {
        ...
        this.gravityForceOnEachModule =
                config.robotMass.in(Kilograms) * 9.8 / moduleSimulations.length;
    }
     ...
     *
     * <p>This method simulates the translational friction forces acting on the robot and applies
     * them to the physics engine. There are two components of the friction forces:
     *
     * <ul>
    ...
    private void simulateChassisFrictionForce() {
        ...
        final Translation2d floorAndModuleSpeedsDiffFieldRelative =
                new Translation2d(
                                differenceBetweenFloorSpeedAndModuleSpeedsRobotRelative
                                        .vxMetersPerSecond,
                                differenceBetweenFloorSpeedAndModuleSpeedsRobotRelative
                                        .vyMetersPerSecond)
        ...
        final Vector2 speedsDifferenceFrictionForce =
                Vector2.create(
                        Math.min(
                                ...
                                totalGrippingForce),
                        MapleCommonMath.getAngle(floorAndModuleSpeedsDiffFieldRelative)
                                .getRadians());
        ...
        final Rotation2d dTheta =
                MapleCommonMath.getAngle(
                                GeometryConvertor.getChassisSpeedsTranslationalComponent(
                                        moduleSpeedsFieldRelative))
        ...
        final Vector2
                ...
                totalFrictionForce =
                        Vector2.create(
                                Math.min(
                                        totalGrippingForce,
                                        totalFrictionForceUnlimited.getMagnitude()),
     ...
     *
     * <p>This method simulates the rotational friction torque acting on the robot and applies them
    ...
    private void simulateChassisFrictionTorque() {
        final double
                desiredRotationalMotionPercent =
                        Math.abs(
                                getDesiredSpeed().omegaRadiansPerSecond
     ...
     *
     * <p>This method returns the desired chassis speeds that the modules are trying to reach. If
     * the robot maintains the current driving voltage and steering position for a long enough
     * period, it will achieve these speeds.
     *
    ...
    public AngularVelocity maxAngularVelocity() {
        return RadiansPerSecond.of(
                maxLinearVelocity().in(MetersPerSecond) / config.driveBaseRadius().in(Meters));
    }
        ...
        return RadiansPerSecondPerSecond.of(
                moduleSimulations[0]
                                .getTheoreticalPropellingForcePerModule(
                                        config.robotMass, moduleSimulations.length)
                                .in(Newtons)
                        * moduleTranslations[0].getNorm()
                        * moduleSimulations.length
