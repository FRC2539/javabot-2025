 ...
 *
 * <p>Check<a
 * href='https://shenzhen-robotics-alliance.github.io/maple-sim/simulating-projectiles/'>Online
 * Documentation</a>
 *
 * <p>The movement is modeled by simple projectile motion.
 *
 * <p>If the projectile flies off the field, touches the ground, or hits its target, it will be
 * automatically removed.
 *
 * <h3>Additional Features:</h3>
 *
 * <ul>
 *   <li>Optionally, it can be configured to become a {@link GamePieceOnFieldSimulation} upon
 *       touching the ground.
 *   <li>Optionally, it can be configured to have a "desired target." Upon hitting the target, it
 *       can be configured to run a callback.
 * </ul>
 *
 * <h3>Limitations:</h3>
 *
...
public class GamePieceProjectile {
    /**
     * This value may seem unusual compared to the standard 9.8 m/s² for gravity. However, through
     * experimentation, it appears more realistic in our simulation, possibly due to the ignoring of
     * air drag.
     */
     ...
     *
     * <p>Optionally, this callback will be used to visualize the projectile flight trajectory in a
     * telemetry system, such as <a
     * href='https://github.com/Mechanical-Advantage/AdvantageScope'>Advantage Scope</a>.
     */
    private Consumer<List<Pose3d>> projectileTrajectoryDisplayCallBackHitTarget =
            projectileTrajectory -> {};

    private Consumer<List<Pose3d>> projectileTrajectoryDisplayCallBackMiss =
            projectileTrajectory -> {};
     ...
     *
     * <p>This value represents the amount of time it takes for the projectile to hit the desired
     * target, calculated when the {@link #launch()} method is called.
     *
     * <p>Determines the results of {@link #hasHitTarget()} and {@link #willHitTarget()}
     *
     * <p>If the projectile never hits the target, or if there is no target, this value remains
     * <code>
     * -1</code>.
     */
     ...
     *
     * @param gamePieceType the type of game piece, which will affect the {@link
     ...
     * @param chassisSpeeds the velocity of the robot chassis when launching the game piece,
     ...
     * @param launchingSpeedMPS the speed at which the game piece is launched, in meters per second
     *     (m/s)
     ...
     *
     * <p>This method calculates the initial velocity of the game piece projectile, accounting for
     * the chassis's translational and rotational motion as well as the shooter's ground speed.
     *
     * @param shooterPositionOnRobot the translation of the shooter on the robot, in the robot's
     *     frame of reference
     ...
     * @param groundSpeedMPS the ground component of the projectile's initial velocity, provided as
     *     a scalar in meters per second (m/s)
     * @return the calculated initial velocity of the projectile as a {@link Translation2d} in
     *     meters per second
     */
    private static Translation2d calculateInitialProjectileVelocityMPS(
        ...
        final Translation2d
                ...
                shooterGroundVelocity =
                        chassisTranslationalVelocity.plus(
                                shooterGroundVelocityDueToChassisRotation);
     ...
     * @param gamePieceType the type of game piece, which will affect the {@link
     *     SimulatedArena#getGamePiecesByType(String)}
     * @param initialPosition the position of the game piece at the moment it is launched into the
     *     air
     * @param initialLaunchingVelocityMPS the horizontal component of the initial velocity in the
     *     X-Y plane, in meters per second (m/s)
     * @param initialHeight the initial height of the game piece when launched (the height of the
     *     shooter from the ground)
     * @param initialVerticalSpeedMPS the vertical component of the initial velocity, in meters per
     *     second (m/s)
     * @param gamePieceRotation the 3D rotation of the game piece during flight (only affects
     *     visualization of the game piece)
     */
    public GamePieceProjectile(
            String gamePieceType,
     ...
     *
     * <ul>
     *   <li>Initiates the projectile motion of the game piece. The current pose can be obtained
     *       with {@link #getPose3d()}.
     *   <li>Calculates whether the projectile will hit the target during its flight. The result can
     *       be obtained using {@link #willHitTarget()}.
     *   <li>Calculates a preview trajectory by simulating the projectile's motion for up to 100
     *       steps, with each step lasting 0.02 seconds.
     *   <li>If specified, displays the trajectory using {@link
     *       GamePieceProjectile#projectileTrajectoryDisplayCallBackHitTarget}, which can be set via
     *       {@link GamePieceProjectile#withProjectileTrajectoryDisplayCallBack(Consumer)}.
     *   <li>Starts the {@link #launchedTimer}, which stores the amount of time elapsed after the
     *       game piece is launched
     * </ul>
     */
    public void launch() {
        ...
        for (int i = 0; i < maxIterations; i++) {
            ...
            if (currentPosition.getZ() < heightAsTouchGround
                    && t * GRAVITY > initialVerticalSpeedMPS) break;
     ...
     *
     * <p>This method determines whether the game piece has flown out of the field's boundaries
     * (outside the fence).
     *
     * <ul>
     *   <li>The result is calculated during the {@link #launch()} method.
     ...
     * </ul>
     *
     * @return <code>true</code> if the game piece has flown out of the field's boundaries,
     ...
     *
     *
     * <h2>Checks if the projectile will hit the target <strong>AT SOME MOMENT</strong> during its
     * flight</h2>
     *
     * <ul>
     *   <li>The result is calculated during the {@link #launch()} method.
     ...
     * <ul>
     *   <li>Before calling {@link #launch()}, this method will always return <code>false</code>.
     *   <li>This is different from {@link #willHitTarget()}, which predicts whether the projectile
     *       will eventually hit the target.
     * </ul>
     *
     * @return <code>true</code> if the projectile has hit the target at the current time, otherwise
     *     <code>false</code>
     */
     ...
     *
     *
     * <h2>Clean up the trajectory through {@link #projectileTrajectoryDisplayCallBackHitTarget}
     * </h2>
     *
    ...
    private Translation3d getPositionAtTime(double t) {
        final double height =
                initialHeight + initialVerticalSpeedMPS * t - 1.0 / 2.0 * GRAVITY * t * t;

        final Translation2d current2dPosition =
                initialPosition.plus(initialLaunchingVelocityMPS.times(t));
     ...
     *
     * <p>The position is calculated using {@link #getPositionAtTime(double)} while the rotation is
     * pre-stored.
     *
     ...
     *
     *
     * <h2>Adds a {@link GamePieceOnFieldSimulation} to a {@link SimulatedArena} to Simulate the
     * Game Piece After Touch-Ground.</h2>
     *
     * <p>The added {@link GamePieceOnFieldSimulation} will have the initial velocity of the game
     * piece projectile.
     *
     * <p>The game piece will start falling from mid-air until it touches the ground.
     *
     * <p>The added {@link GamePieceOnFieldSimulation} will always have collision space on the
     * field, even before touching the ground.
     *
     * @param simulatedArena the arena simulation to which the game piece will be added, usually
     *     obtained from {@link SimulatedArena#getInstance()}
     */
     ...
     *
     * <p>1. If a game piece {@link #hasHitTarget()}, remove it and run {@link #hitTargetCallBack}
     * specified by {@link #withHitTargetCallBack(Runnable)}
     *
     * <p>2. If a game piece {@link #hasHitGround()}, remove it and create a corresponding {@link
     * GamePieceOnFieldSimulation} using {@link #addGamePieceAfterTouchGround(SimulatedArena)}
     *
    ...
    public static void updateGamePieceProjectiles(
        ...
        for (GamePieceProjectile gamePieceProjectile : gamePieceProjectiles) {
            if (gamePieceProjectile.hasHitTarget()
                    ...
                    || gamePieceProjectile.hasGoneOutOfField())
                toRemoves.offer(gamePieceProjectile);
            if (gamePieceProjectile.hasHitTarget()
                    && !gamePieceProjectile.hitTargetCallBackCalled) {
        ...
        while (!toRemoves.isEmpty()) gamePieceProjectiles.remove(toRemoves.poll().cleanUp());
    }
     ...
     *
     *
     * <h2>Configures the Game Piece Projectile to Automatically Become a {@link
     * GamePieceOnFieldSimulation} Upon Touching Ground.</h2>
     *
     * <p>This method configures the game piece projectile to transform into a {@link
     * GamePieceOnFieldSimulation} when it touches the ground.
     *
     ...
     *
     * <p>Reverts the effect of {@link #enableBecomesGamePieceOnFieldAfterTouchGround(Convex,
     * double, double)}.
     */
     ...
     *
     * <p>The method {@link #launch()} will estimate whether or not the game piece will hit the
     * target.
     *
     * <p>After calling {@link #launch()}, {@link #hasHitTarget()} will indicate whether the game
     * piece has already hit the target.
     *
     * <p>Before calling this method, the target position is <code>0, 0, -100 (x,y,z)</code>, which
     * the projectile will never hit.
     *
     * @param targetPositionSupplier the position of the target, represented as a {@link
     *     Translation3d}
     * @return the current instance of {@link GamePieceProjectile} to allow method chaining
     */
     ...
     *
     * <p>Configures the {@link #tolerance} for determining whether the game piece has hit the
     * target. The tolerance defines how close the projectile needs to be to the target for it to be
     * considered a hit.
     *
     ...
     *
     * <p>Sets the {@link #projectileTrajectoryDisplayCallBackHitTarget} to be fed with data during
     * the {@link #launch()} method.
     *
     * <p>A {@link List} containing up to 50 {@link Pose3d} objects will be passed to the callback,
     * representing the future trajectory of the projectile.
     *
     * <p>This is usually for visualizing the trajectory of the projectile on a telemetry, like <a
     * href='https://github.com/Mechanical-Advantage/AdvantageScope'>Advantage Scope</a>
     *
     * @param projectileTrajectoryDisplayCallBack the callback that will receive the list of {@link
     *     Pose3d} objects representing the projectile's trajectory
     * @return the current instance of {@link GamePieceProjectile} to allow method chaining
     */
    public GamePieceProjectile withProjectileTrajectoryDisplayCallBack(
        ...
        this.projectileTrajectoryDisplayCallBackMiss =
                this.projectileTrajectoryDisplayCallBackHitTarget =
                        projectileTrajectoryDisplayCallBack;
        return this;
    }
     ...
     * href='https://github.com/Mechanical-Advantage/AdvantageScope'>Advantage Scope</a>
     *
     * @param projectileTrajectoryDisplayCallBackHitTarget the callback that will receive the list
     *     of {@link Pose3d} objects representing the projectile's trajectory, called if the
     *     projectile will hit the target on its path
     * @param projectileTrajectoryDisplayCallBackHitTargetMiss the callback that will receive the
     *     list of {@link Pose3d} objects representing the projectile's trajectory, called if the
     *     projectile will be off the target
     * @return the current instance of {@link GamePieceProjectile} to allow method chaining
     */
    public GamePieceProjectile withProjectileTrajectoryDisplayCallBack(
        ...
        this.projectileTrajectoryDisplayCallBackMiss =
                projectileTrajectoryDisplayCallBackHitTargetMiss;
        return this;
    }
     ...
     *
     * <p>Sets the {@link #heightAsTouchGround}, defining the height at which the projectile is
     * considered to have landed.
     *
     * <p>When the game piece is below this height, it will either be deleted or, if configured,
     * transformed into a {@link GamePieceOnFieldSimulation} using {@link
     * #enableBecomesGamePieceOnFieldAfterTouchGround(Convex, double, double)}.
     *
     * @param heightAsTouchGround the height (in meters) at which the projectile is considered to
     *     touch the ground
     * @return the current instance of {@link GamePieceProjectile} to allow method chaining
     */
