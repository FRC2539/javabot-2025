 ...
 *
 * <p>This class encapsulates the various configuration parameters required to simulate and control
 * a motor in a system. The configurations include:
 *
 * <ul>
 *   <li><strong>motor:</strong> The motor model used in the simulation (e.g., Falcon 500, NEO).
 *   <li><strong>gearing:</strong> The gear ratio between the motor and the load, affecting the
 *       output torque and speed.
 *   <li><strong>loadMOI:</strong> The moment of inertia (MOI) of the load connected to the motor,
 *       which determines the resistance to changes in rotational speed.
 *   <li><strong>friction:</strong> The torque friction characteristics applied to the motor's
 *       simulation, representing real-world losses.
 *   <li><strong>positionVoltageController:</strong> PID controller for controlling the motor's
 *       position via voltage.
 *   <li><strong>velocityVoltageController:</strong> PID controller for controlling the motor's
 *       velocity via voltage.
 *   <li><strong>positionCurrentController:</strong> PID controller for controlling the motor's
 *       position via current.
 *   <li><strong>velocityCurrentController:</strong> PID controller for controlling the motor's
 *       velocity via current.
 *   <li><strong>feedforward:</strong> A feedforward controller used to compensate for the desired
 *       motor behavior based on input speeds.
 *   <li><strong>forwardHardwareLimit:</strong> The forward limit for motor rotation, specified in
 *       angle units.
 *   <li><strong>reverseHardwareLimit:</strong> The reverse limit for motor rotation, specified in
 *       angle units.
 *   <li><strong>currentLimit:</strong> The current limit applied to the motor to protect it from
...
public final class SimMotorConfigs {
     ...
     *
     * <p>This constructor initializes a {@link SimMotorConfigs} object with the necessary
     * parameters for motor simulation, including the motor model, gearing ratio, load moment of
     * inertia, and friction characteristics.
     *
     * @param motor the motor model to be used in the simulation (e.g., Falcon 500, NEO).
     * @param gearing the gear ratio between the motor and the load, affecting torque and speed
     *     output.
     * @param loadMOI the moment of inertia of the load connected to the motor, representing
     *     rotational resistance.
     * @param frictionVoltage the voltage applied to simulate frictional torque losses in the motor.
     */
    public SimMotorConfigs(
            DCMotor motor, double gearing, MomentOfInertia loadMOI, Voltage frictionVoltage) {
        this.motor = motor;
        this.gearing = gearing;
        this.loadMOI = loadMOI;
        this.friction =
                NewtonMeters.of(motor.getTorque(motor.getCurrent(0, frictionVoltage.in(Volts))));
     ...
     *
     * <p>This method uses the {@link DCMotor} model to find the voltage for a given current and
     * angular velocity.
     *
     ...
     * @return the voltage required for the motor to achieve the specified current and angular
     *     velocity
     */
    public Voltage calculateVoltage(Current current, AngularVelocity mechanismVelocity) {
        return Volts.of(
                motor.getVoltage(
                        current.in(Amps), mechanismVelocity.in(RadiansPerSecond) * gearing));
    }
     ...
     *
     * <p>This method uses the {@link DCMotor} model to find the angular velocity for a given
     * current and voltage.
     *
    ...
    public AngularVelocity calculateMechanismVelocity(Current current, Voltage voltage) {
        return RadiansPerSecond.of(
                        motor.getSpeed(motor.getTorque(current.in(Amps)), voltage.in(Volts)))
                .div(gearing);
    }
     ...
     *
     * <p>This method uses the {@link DCMotor} model to find the current for a given angular
    ...
    public Current calculateCurrent(AngularVelocity mechanismVelocity, Voltage voltage) {
        return Amps.of(
                motor.getCurrent(
                        mechanismVelocity.in(RadiansPerSecond) * gearing, voltage.in(Volts)));
    }
     ...
     *
     * <p>This method uses the {@link DCMotor} model to find the current required for a given
     * torque.
     *
     ...
     *
     * <p>This method uses the {@link DCMotor} model to find the torque generated by a given
     * current.
     *
     ...
     *
     * <p>This method sets the hardware limits for the motor's movement. When either the forward or
     * reverse limit is reached, the motor will be physically restricted from moving beyond that
     * point, based on the motor's hardware constraints.
     *
